          ...
          {watchlist.length === 0 ? (
            ...
            <table className="w-full">
              <thead className="sticky top-0" style={{ background: 'var(--color-panel)' }}>
                <tr>
                  <th className="text-left px-3 py-3 text-[11px] font-semibold text-text-dim uppercase tracking-wide border-b border-border-subtle">Mã</th>
                  <th className="text-right px-2 py-3 text-[11px] font-semibold text-text-dim uppercase tracking-wide border-b border-border-subtle">Giá</th>
                  <th className="text-right px-2 py-3 text-[11px] font-semibold text-text-dim uppercase tracking-wide border-b border-border-subtle">%</th>
                  <th className="px-1 py-3 border-b border-border-subtle w-8" />
                  <th className="px-1 py-3 border-b border-border-subtle w-8" />
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-4">
                        <div className={`text-[15px] font-bold ${isSelected ? 'text-accent' : 'text-text-main'}`}>{item.symbol}</div>
                        <div className="text-[11px] text-text-dim mt-0.5">{item.exchange}</div>
                      </td>
                      <td className={`px-2 py-4 text-right text-[15px] font-mono font-semibold ${chgPct > 0 ? 'text-positive' : chgPct < 0 ? 'text-negative' : 'text-warning'}`}>
                        {q?.price ? toPoint(q.price).toFixed(2) : '—'}
                      </td>
                      <td className={`px-2 py-4 text-right text-[13px] font-mono ${chgPct > 0 ? 'text-positive' : chgPct < 0 ? 'text-negative' : 'text-text-muted'}`}>
                      ...
                      <td className="px-1 py-4">
                        ...
                        >
                          {hasTriggered ? (
                            ...
                            <svg className="w-4.5 h-4.5 text-text-dim hover:text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
                            </svg>
                          )}
                        </button>
                      ...
                      <td className="px-1 py-4">
                        <button
                          ...
                          className="p-1.5 rounded text-text-dim hover:text-negative hover:bg-white/10 transition-colors"
                        >
